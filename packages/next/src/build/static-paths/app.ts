...
export async function buildAppStaticPaths({
  ...
  // Sort the route param keys to ensure a consistent order because iteration on
  // a Map is not guaranteed to be in a consistent order.
  const routeParamKeys = Object.keys(getRouteMatcher(regex)(page) || {}).sort()
